(function($) {
	...
	function bootstrapIE6(el) {
		...
		if ($.eb.ie6()) {
			...
			$('.dropdown-submenu > a', el)
					.after('<span class="dropdown-tri"></span>');

			// / fix multi class selector -- .dropdown-submenu.pull-left
			$('.dropdown-submenu.pull-left', el).removeClass('pull-left')
					.addClass('dropdown-submenu-pull-left');
			// $('.navbar
			// .nav.pull-right').removeClass('pull-right').addClass('nav-pull-right');

			// / fix ul li 100% width bug, set ul width to max width of it's sub
			// li
			...
			// buttons
			// -------------
			var btnColorCls = ['btn-primary', 'btn-warning', 'btn-danger',
					'btn-success', 'btn-info', 'btn-inverse'];
			var btnSizeCls = ['btn-mini', 'btn-small', 'btn-large'];
			$('.btn-group', el).parent().find('.btn-group:eq(0)')
					.addClass('btn-group-first');
			...
			var dropdownPropertyChange = function(e) {
				...
				// / fix ul li 100% width bug, set ul width to max width of it's
				// sub li
				...
				if (self.hasClass('open') && !self.hasClass(cls + '-open')) {
					self.addClass(cls + '-open');
						...
						&& self.hasClass(cls + '-open')) {
					self.removeClass(cls + '-open');
				}

				self.one('propertychange', {
							cls : cls
						}, dropdownPropertyChange);
			};
			$.each(['btn-group', 'dropdown'], function(k, cls) {
						$('.' + cls, el).one('propertychange', {
									cls : cls
								}, dropdownPropertyChange);
					});
			...
			$('.form-horizontal .controls:first-child', el)
					.addClass('controls-first-child');
			...
			// fix tag[disabled]
			$('input[disabled], select[disabled], textarea[disabled]', el)
					.each(function() {
						var self = $(this);
						self.addClass(self[0].tagName.toLowerCase()
								+ '-disabled');
					});
			...
			// }
			// else if (typeof self.attr('readonly') == 'undefined' &&
			// self.hasClass(cls+'-readonly')) {
			...
			// fix tag[readonly]
			$('input[readonly], select[readonly], textarea[readonly]', el)
					.each(function() {
						var self = $(this);
						self.addClass(self[0].tagName.toLowerCase()
								+ '-readonly');
			...
			$(
					...
					el).each(function() {
						var self = $(this);
						self.addClass(self.attr('type').toLowerCase()
								+ '-disabled');
					});
			...
			$.each(ctlGrpTypeCls, function(k, v) {
						$('.control-group.' + v, el).addClass('control-group-'
								+ v);
					});
			...
			$('.control-group', el).one('propertychange',
					controlGroupPropertyChange);

			// -------------
			// popover
			// -------------
			// $('.popover .arrow', el).after('<span
			// class="arrow-after"></span>');
			...
			$('[class^="icon-"],[class*=" icon-"]').each(function() {
				...
				if (!self.hasClass('icon-xxx')) {
					...
					self.css('background-position-y', (parseInt(self
									.css('background-position-y')) + 1)
									+ 'px');
				}
			});
			...
			// carousel
			// -------------
			$('.carousel-control.left', el).removeClass('left')
					.addClass('carousel-control-left');
			$('.carousel-control.right', el).removeClass('right')
					.addClass('carousel-control-right');
			$('.carousel-caption').each(function() {
				var self = $(this);
				var padding = self.outerWidth() - self.width();
				self.width(self.parents('.carousel-inner .item').width()
						- padding);
			});
